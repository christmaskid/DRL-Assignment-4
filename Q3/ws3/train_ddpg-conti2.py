...
class DDPGAgent:
    ...
    def act(self, obs, deterministic=False):
        ...
        if not deterministic:
            noise = torch.distributions.Normal(0, 0.1).rsample().to(self.device)
...
def train():
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        if (episode+1)%10 == 0:
            ...
            plt.savefig('training_progress_ddpg-conti2.png')
            plt.close()
