...
class Actor(nn.Module): # GaussianPolicy
    ...
    def sample(self, state, eps=1e-6):
        ...
        log_prob = distribution.log_prob(x_t).sum(-1, keepdim=True) - torch.log(1 - y_t.pow(2) + eps).sum(-1, keepdim=True)
...
class SACAgent:
    def __init__(self, state_dim, act_dim, max_action, hidden_dim=256, lr=1e-5, device="cuda"):
    ...
    def save(self):
        ...
        }, "ckpt_q3_sac_fixed2.pt")
    
    def load(self, ckpt_name="ckpt_q3_sac_fixed2.pt"):
...
def train(load_ckpt=None):
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        if (episode+1)%10 == 0:
            ...
            plt.savefig('training_progress_sac_fixed2.png')
            plt.close()
